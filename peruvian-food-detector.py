...
classes = ['aji_gallina',
  ...
  'rocoto_relleno',
  'tacu_tacu']
data = ImageDataBunch.single_from_classes(path, classes, tfms=get_transforms(), size=512).normalize(imagenet_stats)
learner = create_cnn(data, models.resnet50, metrics=error_rate, pretrained=False)
learner.unfreeze()
#learner.load("model-weights");
...
def form(request):
    return HTMLResponse(
        """
        <h1>Peruvian food detector!</h1>
        <p>Find out if the food belongs to one of the following categories:
		
		[Ceviche, Ají de gallina, Lomo saltado, Causa limeña, Rocoto relleno, Tacu tacu, Anticuchos, Pan con chicharrón, Olluquito, Picante de cuy]
		
		
		Upload image or specify URL.</p><br>
